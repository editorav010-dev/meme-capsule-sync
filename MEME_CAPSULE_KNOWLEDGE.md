...
ECOSYSTEM STRUCTURE:
1. PUBLIC MOBILE APP (com.meme.capsule - v3.0 / versionCode 21):
   ...
   - Active Session Persistence & Background Share State Retention (v3.0): Synchronous snapshot persistence and instant hydration (`localStorage` key `'meme_capsule_active_session_v1'`) preserves `meme`, `screen`, `memeQueue` (7-meme buffer), `memeHistory`, `historyIndex`, `activeTab`, `multiDropMemes`, and `memeDimensionCache` across external app sharing, backgrounding, and OS Activity recreation. Coupled with `MainActivity.java` `WebSettings.LOAD_DEFAULT` cache mode, `onPause` synchronous `meme-capsule-persist-session` flush, `Intent.FLAG_ACTIVITY_NEW_DOCUMENT` on external share intents, and `MemeCard.tsx` background resume recovery so memes never disappear or reload when returning to the app.
